...
namespace DAO
{
    public class NhanVien_DAO
    {
        ...
        {
            string sTruyVan = string.Format(@"insert into nhanvien values(N'{0}',N'{1}',N'{2}',N'{3}','{4}',N'{5}')", nv.SMaNV, nv.SHoLot, nv.STen, nv.SPhai, nv.DtNgaySinh.ToString("yyyy/MM/dd"), nv.SMaCV);
            con = DataProvider.MoKetNoi();
        ...
        public static bool SuaNhanVien(NhanVien_DTO nv) {
            string sTruyVan = string.Format(@"update nhanvien set holot=N'{0}',tennv=N'{1}',phai=N'{2}',ngaysinh='{3}',macv=N'{4}' where manv=N'{5}'", nv.SHoLot, nv.STen, nv.SPhai, nv.DtNgaySinh.ToString("yyyy/MM/dd"), nv.SMaCV, nv.SMaNV);
            con = DataProvider.MoKetNoi();
            bool kq = DataProvider.TruyVanKhongLayDuLieu(sTruyVan, con);
            DataProvider.DongKetNoi(con);
            return kq;
